---
name: code-review
description: "Review pull requests and diffs in ronin-to-koinly — the Go CLI that exports Ronin wallet transactions to Koinly CSV — against the rios0rios0/guide standards: the CSV contract, error handling at the entry point, and changelog discipline. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `ronin-to-koinly`

`ronin-to-koinly` fetches transactions from the Ronin wallet API and writes a Koinly-compatible CSV for crypto tax reporting. The output feeds a tax filing, so column order, date format, and amount precision are correctness concerns, not formatting preferences.
...
## What matters most in `ronin-to-koinly`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **The CSV contract is the product.** The `Transaction` struct mirrors Koinly's column layout (Date, SentAmount/Currency, ReceivedAmount/Currency, FeeAmount/Currency, Tag). Reordering, renaming, or dropping a column silently corrupts every import — **Critical**, and it needs a changelog entry flagged as breaking.
- **Dates are formatted `2006-01-02 15:04:05`** to match what Koinly expects. A change to the layout string, or a timezone conversion introduced along the way, changes the meaning of every row.
- **Amount precision must not go through `float64` formatting** that can round or emit scientific notation — check any new arithmetic on amounts and fees.
- **The output path is fixed**: `koinly_transactions.csv` in the working directory. Changing it is a user-visible change that needs README and changelog updates.
- **The tree is deliberately shallow.** All logic lives in `cmd/ronin-to-koinly/main.go`; `internal/` and `test/` are scaffolded for growth. Introducing a package is fine — introducing a framework or a DI container for a single-file CLI is not.
- `log.Fatalf` is acceptable at the CLI entry point only; anything library-shaped returns an error. Every exported type and function needs a doc comment.
- Prefer the standard library; a new third-party dependency needs justification in the pull-request description.
...
- There is no test suite yet — a pull request that adds behaviour should start one under `test/`, following the BDD structure below.
- API responses belong in fixtures; a test that calls the live Ronin API is not a unit test.
...
- The single-file layout — it is intentional at this size.
- The empty `internal/` and `test/` directories.
